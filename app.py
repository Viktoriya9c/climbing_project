...
import os
import cv2 
import gc # Модуль для принудительной очистки оперативной памяти
...
with col_video:
    st.subheader("🎥 Плеер")
    if final_video_path and not st.session_state['is_analyzing']:
        st.video(final_video_path, start_time=st.session_state['start_sec'])
    elif video_url:
        st.video(video_url)
    else:
        st.info("Загрузите видео в боковом меню")
...
with col_results:
    ...
    # Высота 400px - оптимально, чтобы быть вровень с плеером
    with st.container(height=400, border=True):
        live_list_placeholder = st.empty()
        results = st.session_state.get('found_timestamps', [])
        
        if not st.session_state['is_analyzing'] and results:
            with live_list_placeholder.container():
                # Сортировка по фамилии
                sorted_results = sorted(results, key=lambda x: x['name'])
                options = ["--- Перейти к участнику ---"] + \
                          [f"⏱️ {i['time']} — {i['name']} (№{i['num']})" for i in sorted_results]
                
                selected = st.selectbox("Навигация:", options=options, index=0)
                if selected != options[0]:
                    time_part = selected.split(" — ")[0].replace("⏱️ ", "")
            ...
            pass # Живой вывод будет здесь во время цикла
        else:
            live_list_placeholder.write("Тут появятся результаты...")
...
if st.session_state['is_analyzing']:
        ...
        if need_download:
            try:
                def ui_update(pct):
                    ...
                    status_place.text(f"📥 Загрузка видео: {int(pct*100)}%")
    ...
    if csv_path and final_video_path:
        ...
        if not cap.isOpened():
            st.error("❌ Ошибка открытия файла.")
            st.session_state['is_analyzing'] = False
        else:
            ...
            while cap.isOpened():
                ...
                status_place.text(f"🔍 Анализ: {time_str}")
                if duration > 0:
